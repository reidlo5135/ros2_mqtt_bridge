...
#define RCLCPP_NODE_POINTER_VALIDATION(rcl_node_ptr, rcl_target_connection)                                                     \
    ...
    {                                                                                                                           \
        RCUTILS_LOG_ERROR_NAMED(RCL_NODE_NAME, "rcl register [%s] - rclcpp::Node pointer is nullptr\n", rcl_target_connection); \
        RCLCPP_LINE_ERROR();                                                                                                    \
        return nullptr;                                                                                                         \
    }

#define RCLCPP_REGISTER_INFO(rcl_node_ptr, rcl_target_connection, rcl_target_connection_name)                                      \
    RCUTILS_LOG_INFO_NAMED(RCL_NODE_NAME, "rcl register [%s] named with [%s]", rcl_target_connection, rcl_target_connection_name); \
    RCLCPP_LINE_INFO();
